...
def main(domains: int = 50, path: Path = SCENRIOS_PATH / "y2022"):
    path = path.absolute()
...
class Profile:
    ...
    def from_file(cls, utility_file):
        ...
        for issue, values in raw["issueUtilities"].items():
            issue_value_weights = {
                v: w
                for v, w in values["DiscreteValueSetUtilities"][
                    "valueUtilities"
                ].items()
            }
    ...
    def to_negmas(self) -> LinearAdditiveUtilityFunction:
        issue_names = list(
            self.profile["LinearAdditiveUtilitySpace"]["domain"]["issuesValues"].keys()
        )
        os = make_os(
            [
                make_issue(name=k, values=v["values"])
                for k, v in self.profile["LinearAdditiveUtilitySpace"]["domain"][
                    "issuesValues"
                ].items()
            ],
            name=self.profile["LinearAdditiveUtilitySpace"]["domain"]["name"],
        )
        assert all(_.name == iname for _, iname in zip(os.issues, issue_names))
        return LinearAdditiveUtilityFunction(
            values=tuple(  # type: ignore
                TableFun(
                    self.profile["LinearAdditiveUtilitySpace"]["issueUtilities"][_][
                        "DiscreteValueSetUtilities"
                    ]["valueUtilities"]
                )
                for _ in issue_names
            ),
            weights=[
                self.profile["LinearAdditiveUtilitySpace"]["issueWeights"][_]
                for _ in issue_names
            ],
            outcome_space=os,
            reserved_value=0.0,
            name=self.profile["LinearAdditiveUtilitySpace"]["name"],
        )
...
class Domain:
    ...
    def to_negmas(self) -> Scenario:
        issue_names = list(self.domain["issuesValues"].keys())
        os = make_os(
            [
                make_issue(name=k, values=v["values"])
                for k, v in self.domain["issuesValues"].items()
            ],
            name=self.domain["name"],
        )
        assert all(_.name == iname for _, iname in zip(os.issues, issue_names))
        return Scenario(
            ufuns=(self.profile_A.to_negmas(), self.profile_B.to_negmas()),
            agenda=os,
            mechanism_type=SAOMechanism,
            mechanism_params=dict(time_limit=180),
        )
    ...
    def to_file(self, parent_path):
        ...
        self.to_negmas().to_genius_folder(
            Path(parent_path).absolute() / self.domain["name"]
        )
